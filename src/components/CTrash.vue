<template>
  <c-form :form-name="'Deleted photo'" @deletedPhoto="getDeletedPhoto($event)"></c-form>
      ...
      <div class="col-12 col-md-4 mb-3" v-for="photo in myPhotos.user.photos">
        <c-card show-photo-header="showPhotoHeader">
          <template v-slot:header class="justify-content-between">
            <span><c-image :src="myPhotos.user.profile.file_url" :styl="'img-fluid rounded-circle border mx-auto'"/></span> <span> {{myPhotos.user.name}}</span><span class="float-md-end"><i @click="forceDelete(photo.id)" class="fa fa-trash"></i></span>
          </template>
...
export default {
  ...
  methods:{
    ...
    async forceDelete(photoId){
      this.$store.dispatch('setForceDeletePhotoUrlInMutations' , photoId)
      let res = await fetch(this.$store.state.url_my_photo_force_delete, {
        method: 'DELETE',
        headers:{
